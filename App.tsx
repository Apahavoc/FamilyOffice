...
const App: React.FC = () => {
  ...
  const NavItem = ({ tab, label, icon }: { tab: TabView; label: string; icon: React.ReactNode }) => (
    <button
    ...
    <div className="flex h-screen bg-slate-950 text-slate-50 overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          <NavItem tab={TabView.DASHBOARD} label="Dashboard Global" icon={<LayoutDashboard className="w-5 h-5" />} />
          <NavItem tab={TabView.REAL_ESTATE} label="Inmobiliario" icon={<Building className="w-5 h-5" />} />
          <NavItem tab={TabView.PORTFOLIO} label="Cartera de Valores" icon={<Briefcase className="w-5 h-5" />} />
          <NavItem tab={TabView.TREASURY} label="Tesorería" icon={<Landmark className="w-5 h-5" />} />
        </nav>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950">
        ...
        <main className="flex-1 overflow-y-auto p-8 relative">
           ...
           <div className="max-w-7xl mx-auto">
             ...
             {activeTab === TabView.REAL_ESTATE && <RealEstate />}
             {activeTab === TabView.PORTFOLIO && <Portfolio />}
             {activeTab === TabView.TREASURY && (
               <div className="flex flex-col items-center justify-center h-[500px] text-slate-600 border border-dashed border-slate-800 rounded-2xl animate-fade-in">
                  <Landmark className="w-16 h-16 mb-4 opacity-20" />
                  <p className="text-lg font-medium">Módulo de Tesorería</p>
                  <p className="text-sm">Vista detallada de liquidez y cash flow en desarrollo.</p>
               </div>
             )}
           </div>
        </main>
